...
- Inventory (/admin/inventory): the live stock. Add a car with photos/specs/price, publish to put it on the site, unpublish to pull it, and mark it sold when it's gone. There's also a one-click duplicate-as-draft (PPSR and service history reset on the copy). On the listing form, "Fill from photos" reads the uploaded photos and starts the specs (make, model, body type, colour — only what a photo can show, and only into empty fields), and "Draft with AI" under Listing copy writes a first draft of the description from the photos and the specs; both are guesses to check, nothing saves until the form is saved, and "Undo draft" puts back whatever was there.
