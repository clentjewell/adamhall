...
export default function CarCard({ car, priority = false }: { car: Car; priority?: boolean }) {
  ...
  return (
    <Link
      href={`/cars/${car.slug}`}
      className="bg-card rounded-2xl border border-transparent overflow-hidden group hover:border-forest-200 hover:shadow-lg hover:shadow-stone-300/50 hover:-translate-y-1 transition-all duration-300"
    >
